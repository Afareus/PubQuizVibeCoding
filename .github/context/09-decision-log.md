...
### D-069 — R10: In-memory reconnect metriky a diagnostické endpointy
- **Datum/čas (UTC):** 2026-04-03T00:00:00Z
- **Krok:** R10
- **Rozhodnutí:** Reconnect observabilita je implementována jako in-memory singleton `ReconnectMetrics` s thread-safe čítači (team/organizer reconnect, snapshot served, duplicate submit retry, failed resync, resync duration) a kruhovou frontou posledních 200 eventů. Diagnostické endpointy `GET /api/diagnostics/reconnect-metrics` a `POST .../reset` zpřístupňují metriky bez externí závislosti. Strukturované logy s `SessionId`, `TeamId`, `ConnectionId`, `SnapshotVersion` a `ResyncMs` jsou doplněny do `SessionParticipationService`, `SessionHub`, `SessionRealtimePublisher` a `SessionProgressionBackgroundService`. Alert prahy jsou dokumentovány v XML komentářích `IReconnectMetrics`.
- **Důvod:** Specifikace R10 vyžaduje metriky, strukturované logy a alert prahy; in-memory singleton je nejjednodušší řešení konzistentní s MVP bez zavedení externího monitorovacího stacku.
- **Dopad:** Operátor může za běhu kontrolovat reconnect zdraví přes REST endpoint; logy jsou použitelné pro centralizovaný sběr (ELK/Seq) bez dalších změn; čítače se resetují při restartu procesu (akceptovatelné pro MVP).
